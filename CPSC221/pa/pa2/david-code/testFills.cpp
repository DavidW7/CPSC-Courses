...
TEST_CASE("fill::basic custom bfs", "[weight=1][part=fill]") {
	...
	anim = filler::vorCustomBFS(img, DENSITY,
		GRIDFRAMEFREQ);

	PNG result = anim.write("images/bfscustomleaf.gif");
	result.writeToFile("images/bfscustomleaf.png");
}
TEST_CASE("fill::basic custom dfs", "[weight=1][part=fill]") {
	...
	anim = filler::vorCustomDFS(img, DENSITY,
		GRIDFRAMEFREQ);

	PNG result = anim.write("images/dfscustomleaf.gif");
	result.writeToFile("images/dfscustomleaf.png");
}
