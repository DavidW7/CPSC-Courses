...
private:
	...
	public:
		// Node constructors
		Node(PNG & imIn, pair<int, int> ul, int size, Node *par, int l);

		pair<int, int> upLeft;	// upper-left coords of Node's subimage
		//upLeft.first = horizontal
		//upLeft.second = vertical
		...
		int loc; //location relative to parent
		//0 = nw of parent
		//1 = ne of parent
		//2 = sw of parent
		//3 = se of parent
...
private:
	...
	std::priority_queue<Node*, std::vector<Node*>, compareNode> varQueue;
	//std::priority_queue<Node*, std::vector<Node*>, decltype(QTree::&compareNode)> varQueue;

	void splitTwoNodes(Node* n, Node* m);
	bool isSplitableLen(Node* n); // check if node has splitable side len (>= 2)
	Node* copyRecursively(Node* n);
	void deleteRecursively(Node* n);
	void writeRecursively(Node* n);
	void constructs();
