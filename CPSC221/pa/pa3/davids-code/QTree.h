...
class QTree {
 ...
 public:
  ...
  // Helper function to recursively split neighbours that
  // needs to be splitted so balance is maintained
  void splitNeighboursToMaintainBalance(Node * p);

  void createChildren(Node * t);

  // Helper function for creating the QTree
  void createQTree(PNG & imIn);

  // Helper function for traversing tree and coloring 
  void traverseTreeAndColor(Node* t);
