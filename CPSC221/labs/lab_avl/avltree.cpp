...
void AVLTree<K, V>::rebalance(Node*& subtree)
{
	...
	if (subtree->height <= 1) {
		...
		int diff = height(subtree->right) - height(subtree->left);
		if (diff >= 2) {
...
void AVLTree<K, V>::remove(Node*& subtree, const K& key)
{
    ...
    if (key < subtree->key) {
        ...
        if (subtree->left == NULL && subtree->right == NULL) {
			...
			Node *tempBeforePred = subtree;
			Node *tempPred = tempL;
			while (tempPred->right != NULL) {
				tempBeforePred = tempPred;
				tempPred = tempPred->right;
			}
			swap(subtree, tempPred);
			if (tempPred->left != NULL) {
				tempBeforePred->right = tempPred->left;
			}
			delete tempPred;
