...
PNG createSpotlight(PNG image, int centerX, int centerY) {
	...
	for (unsigned x = 0; x < image.width(); x++) {
		for (unsigned y = 0; y < image.height(); y++) {
			HSLAPixel *pixel = image.getPixel(x, y);
			double xDist = fabs(centerX - (int)x);
			double yDist = fabs(centerY - (int)y);
			...
			if (distance < 0) {
				...
				newLum = 1 - distance * 0.005;
				if (newLum < 0) {
					newLum = 0;
				}
			}
			pixel->l = pixel->l * newLum;
		}
	}
...
PNG ubcify(PNG image) {
	...
	for (unsigned x = 0; x < image.width(); x++) {
		for (unsigned y = 0; y < image.height(); y++) {
			HSLAPixel *pixel = image.getPixel(x, y);
			double distanceToYellow = fabs(ubcyellow - pixel->h);
			double distanceToBlue = fabs(ubcblue - pixel->h);
			if (distanceToBlue < distanceToYellow) {
				pixel->h = ubcblue;
			}
			else {
				pixel->h = ubcyellow;
	...
	for (unsigned x = 0; x < secondImage.width(); x++) {
		for (unsigned y = 0; y < secondImage.height(); y++) {
			...
			if (secondPixel->l == 1) {
				...
				if (firstPixel->l * 1.2 < 1) {
					firstPixel->l = firstPixel->l * 1.2;
				}
				else {
					firstPixel->l = 1;
				}
