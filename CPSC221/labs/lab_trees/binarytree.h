...
class BinaryTree
{
  ...
  private:
	...
	//my functions
	void printLeftToRightHelper(const Node *n) const;
	void mirrorHelper(Node *n);
	void printPathsHelper(Node *n, string s) const;
	int sumDistancesHelper(Node *n, int lenAccu) const;
	bool isOrderedHelper(Node* n) const;
};
