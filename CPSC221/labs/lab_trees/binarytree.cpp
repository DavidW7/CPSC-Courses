...
template <typename T>
void BinaryTree<T>::printLeftToRightHelper(const Node *n) const 
{
	//base
	if (n->left != NULL) {
		printLeftToRightHelper(n->left);
	}
	cout << to_string(n->elem);
	if (n->right != NULL) {
		printLeftToRightHelper(n->right);
	}
}
...
template <typename T>
void BinaryTree<T>::printPathsHelper(Node *n, string s) const
{
	string temp = s + " " + to_string(n->elem);
	if (n->left == NULL && n->right == NULL) {
		cout << temp << '\n';
	}
	else if (n->left != NULL) {
		printPathsHelper(n->left, temp);
	}
	else if (n->right != NULL) {
		printPathsHelper(n->right, temp);
	}
}
...
{
	return 0 + sumDistancesHelper(root->left, 0) + sumDistancesHelper(root->right, 0);
}

template <typename T>
int BinaryTree<T>::sumDistancesHelper(Node *n, int lenAccu) const 
{
	if (n == NULL) {
		return 0;
	}
	else {
		return lenAccu + 1 + sumDistancesHelper(n->left, lenAccu + 1) + sumDistancesHelper(n->right, lenAccu + 1);
	}
}
...
template <typename T>
bool BinaryTree<T>::isOrderedHelper(Node* n) const
{
