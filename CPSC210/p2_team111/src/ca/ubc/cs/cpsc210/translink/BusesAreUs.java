...
public class BusesAreUs extends Activity implements LocationListener, StopSelectionListener {
    ...
    public void onLocationChanged(Stop nearest, LatLon locn) {
        if (nearest != null) {
            ...
            location.setLatitude(nearest.getLocn().getLatitude());
            location.setLongitude(nearest.getLocn().getLongitude());
        }
    }
    ...
    /**
     * Download arrivals and bus data for stop selected by user;
     *
     * @param stop stop selected by user
     */
    @Override
