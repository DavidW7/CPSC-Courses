...
public class StopManager implements Iterable<Stop> {
    ...
    public Stop getStopWithNumber(int number) {
        ...
        if (stopMap.containsKey(number)) {
            newStop = stopMap.get(number);
        } else {
            newStop = new Stop(number, "", new LatLon(0, 0));
    ...
    public Stop getStopWithNumber(int number, String name, LatLon locn) {
        ...
        if (stopMap.containsKey(number)) {
            ...
            newStop.setLocn(locn);
        } else {
            newStop = new Stop(number, name, locn);
            stopMap.put(number, newStop);
        }
        return newStop;
    }
    ...
    public void setSelected(Stop selected) throws StopException {
        if (!stopMap.containsValue(selected)) {
            throw new StopException("stop manager doesn't contain selected stop");
        } else {
    ...
    public Stop findNearestTo(LatLon pt) {
        ...
        Stop shortestDistStop = null;
        for (Stop s : stopMap.values()) {
            double distanceBtw = distanceBetween(s.getLocn(), pt);
            if (distanceBtw < RADIUS && distanceBtw < shortestDistance) {
                shortestDistance = distanceBtw;
                shortestDistStop = s;
            }
        }
        return shortestDistStop;
    }
