...
public class ArrivalsParser {
    ...
    public static void parseArrivals(Stop stop, String jsonResponse)
        ...
        for (int index = 0; index < routes.length(); index++) {
            ...
            parseRoutes(stop, route);
        }
    }

    private static void parseRoutes(Stop stop, JSONObject routes) throws JSONException, ArrivalsDataMissingException {
        if (routes.has("RouteNo") && routes.has("Schedules")) {
            String routeNo = routes.getString("RouteNo");
            Route r = RouteManager.getInstance().getRouteWithNumber(routeNo);
            stop.addRoute(r);

            JSONArray arrivals = routes.getJSONArray("Schedules");

            for (int index = 0; index < arrivals.length(); index++) {
                JSONObject arrival = arrivals.getJSONObject(index);
                parseArrival(stop, r, arrival);
            }
            ...
            if (count == 0) {
                throw new ArrivalsDataMissingException("No arrival added to stop");
            }
        }
    }

    private static void parseArrival(Stop stop, Route route, JSONObject arrival) throws JSONException {
        if (arrival.has("ExpectedCountdown")
                && arrival.has("Destination")
                && arrival.has("ScheduleStatus")) {
            int timeToStop = arrival.getInt("ExpectedCountdown");
            String dest = arrival.getString("Destination");
            String status = arrival.getString("ScheduleStatus");

            Arrival a = new Arrival(timeToStop, dest, route);
            a.setStatus(status);
            stop.addArrival(a);
        }
    }
