...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
	/*
  int increase_out = GROWTH_FACTOR;
  for (int count = 0; in_list->len > count; count++) {
	  if (increase_out == GROWTH_FACTOR) {
		expand_list(out_list);
	  }
	  increase_out--;
	  if (increase_out == 0) {
		  increase_out = GROWTH_FACTOR;
	  }
	  
	  list_append(out_list, (element_t)*(f(NULL, in_list[count])));
  }
  */
  for (int count = 0; in_list->len > count; count++) {
	element_t e = NULL;
	f(&e, in_list->data[count]);
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  ...
  if (in_list0->len < in_list1->len) {
    min_len = in_list0->len;
  } else {
    min_len = in_list1->len;
  }
  int increase_out = GROWTH_FACTOR;
  for (int count = 0; min_len > count; count++) {
	  if (increase_out == GROWTH_FACTOR) {
		expand_list(out_list);
	  }
	  increase_out--;
	  ...
	  list_append(out_list, (element_t)*(f(NULL, in_list0[count], in_list1[count])));
  }
  */
  int min_len;
  if (in_list0->len < in_list1->len) {
  ...
  for (int count = 0; min_len > count; count++) {
	element_t e = NULL;
	f(&e, in_list0->data[count], in_list1->data[count]);
	list_append(out_list, e);
  }
}
...
void list_foldl (void (*f) (element_t*, element_t, element_t), element_t* out_element_p,  struct list* in_list) {
  for (int count = 0; in_list->len > count; count++) {
	f(out_element_p, *out_element_p, in_list->data[count]);
  }
}
...
void list_filter (int (*f) (element_t), struct list* out_list, struct list* in_list) {
  for (int count = 0; in_list->len > count; count++) {
	if (f(in_list->data[count]) == 1) {
	  element_t *e = &in_list->data[count];
