...
int exec() {
  int cont = 1;
  int addr, val;
  void *jt1[0x10] = {&&L1_0, &&L1_1, &&L1_2, &&L1_3, &&L1_4, &&Def_1, &&L1_6, &&L1_7, &&L1_8, &&L1_9, &&L1_a, &&L1_b, &&L1_c, &&Def_1, &&Def_1, &&L1_f};
  if (insOpCode > 0xf || insOpCode < 0) goto Def_1;
  goto *jt1[insOpCode];
  L1_0: // ld $i, d .............. 0d-- iiii iiii
    reg [insOp0] = insOpExt;
    goto cont1;
  L1_1: // ld o(rs), rd .......... 1osd
    addr = (insOp0 << 2) + reg [insOp1];
    reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
    goto cont1;
  L1_2: // ld (rs, ri, 2), rd .... 2sid
    addr = reg [insOp0] + (reg [insOp1] << 2);
    reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
    goto cont1;
  L1_3: // st rs, o(rd) .......... 3sod
  ...
  L1_4: // st rs, (rd, ri, 4) .... 4sdi
    ...
    goto cont1;
  L1_6: ;// ALU ................... 6-sd
    void *jt0[0x10] = {&&L0_0, &&L0_1, &&L0_2, &&L0_3, &&L0_4, &&L0_5, &&L0_6, &&L0_7, &&Def_0, &&Def_0, &&Def_0, &&Def_0, &&Def_0, &&Def_0, &&Def_0, &&L0_f};
    if (insOp0 > 0xf || insOp0 < 0) goto Def_0;
	goto *jt0[insOp0];
    L0_0: // mov rs, rd ........ 60sd
