...
uthread_sem_t cantProduce;
uthread_sem_t cantConsume;
	...
	for (int i = 0; i < NUM_ITERATIONS; i++) {
		while (items + 1 > max_items) {
			...
			if (state == 0) {
				...
				uthread_sem_signal(cantConsume);
				state = 1;
			}

			uthread_sem_wait(cantProduce);
		}
		...
		*/
		...
		if (i + 1 == NUM_ITERATIONS) {
			...
			if (num_consumers == consumersDone) {
				...
				uthread_sem_signal(cantProduce);
			}
			else {
...
int main(int argc, char **argv) {
	...
	cantProduce = uthread_sem_create(1);
	cantConsume = uthread_sem_create(1);
	...
	uthread_sem_destroy(cantProduce);
	uthread_sem_destroy(cantConsume);
